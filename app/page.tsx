...
//         }
//       }
//     }, isDeleting ? 40 : 10); // Faster deletion speed
...
//                 target="_blank"
//                 className="p-3 bg-primary/80 text-primary-foreground rounded-full hover:scale-110 transition-all ease-in-out duration-300"
//                 aria-label="LinkedIn"
//               >
//                 <Linkedin size={28} />
//               </Link>
...
//               <Link
//                 href="/projects"
//                 className="inline-flex items-center gap-2 bg-primary/80 text-primary-foreground px-6 py-3 rounded-lg hover:glow transition-all"
//               >
//                 View Work
//                 <ArrowRight size={20} />
//               </Link>
...
export default function Home() {
  ...
  const designations: string[] = ["MERN STACK Developer", "Programming Enthusiast", "Adaptive and Reliable"];
  const [designationIndex, setDesignationIndex] = useState<number>(0);
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    const currentDesignation = designations[designationIndex];
    const duration = isDeleting ? 40 : 120;
    ...
    if (progress < 1) {
      ...
      if (!isDeleting) {
        if (displayText.length < currentDesignation.length) {
          ...
          if (displayText.length + 1 === currentDesignation.length) {
            setTimeout(() => setIsDeleting(true), 1500);
          }
        }
      } else {
